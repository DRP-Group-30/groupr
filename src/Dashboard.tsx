...
const Dashboard = () => {
  ...
  async function getMatched() {
    ...
    const matched: string[] = await Promise.all(
      defaultUser
        .get("Matched")
        .map((ref: DocumentReference) =>
          getDoc(ref).then((doc) => doc.get("Name"))
        )
    );
    setMatched(matched);
  }
  ...
  async function getInterested() {
    const defaultUser = await getDoc(doc(db, "users", DEFAULT_USER_ID));
    const interested: string[] = await Promise.all(
      defaultUser
        .get("Interested")
        .map((ref: DocumentReference) =>
          getDoc(ref).then((doc) => doc.get("Name"))
  ...
  async function getRejected() {
    const defaultUser = await getDoc(doc(db, "users", DEFAULT_USER_ID));
    const rejected: string[] = await Promise.all(
      defaultUser
        .get("Rejected")
        .map((ref: DocumentReference) =>
          getDoc(ref).then((doc) => doc.get("Name"))
